...
Reddit (r/wallstreetbets, r/stocks, r/investing RSS) was removed on
2026-09-11: the per-IP rate limit 429s every request from this host, so
the feed never carried data — only log noise each cycle.
...
def create_sentiment_analyst(llm):
    ...
    def sentiment_analyst_node(state):
        ...
        # Pre-fetch both sources. Each fetcher degrades gracefully and
...
5. **Be honest about data limits.** If StockTwits returned only a handful of messages, or one or more sources returned an "<unavailable>" placeholder, the sentiment read is less robust — flag this explicitly in the `confidence` field and the narrative.
